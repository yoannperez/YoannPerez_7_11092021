...
exports.login = (req, res) => {
	...
	db.User.findOne({where: {email: req.body.email}})
		...
		.catch((error) =>
			res.status(500).json({error: "Server error, POST message invalid, check email !"})
		);
};
		...
		.then((user) => {
			...
			if (owner && !lastUser) {
				db.User.destroy({where: {id: req.params.id}})
					.then(function (data) {
						res.status(200).json({
							token: jwt.sign({userId: user.id}, process.env.TOKEN_KEY, {expiresIn: "1ms"}),
